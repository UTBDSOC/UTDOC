...
import { Bell, MessageSquare, Mail, ShieldAlert, User, CheckCircle2, X } from 'lucide-react'
...
export default function SettingsPage() {
  ...
  const handleAvatarChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      // Create a temporary object URL for the image preview
  ...
  const handleDeleteAccount = () => {
    ...
    setTimeout(() => {
      ...
      alert("Account deletion request submitted to admins.")
    }, 1000)
  }
            ...
            <section className="bg-bg-card border border-status-purple/20 rounded-2xl p-6 md:p-8 shadow-xl relative overflow-hidden animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2">
                  ...
                  >
                    {isTestingWebhook ? 'Testing...' : testSuccess ? <><CheckCircle2 className="w-4 h-4" /> Success</> : 'Test Connection'}
                  </button>
                </div>
              </div>
            </section>
